...
//			}
			...
			double correctionError = 1.0; // A 1 inch tolerance for position of swerve modules
			...
			if (Math.hypot(fieldX, fieldY) > (DriveTrain.getRadius() / 2) + correctionError || Math.hypot(fieldX, fieldY) < (DriveTrain.getRadius() / 2) - correctionError) {

				// Gets module num for calculation of position below
				double moduleNum = Math.floor((Gyro.getUnitCircleAngle() - (Math.atan(fieldY / fieldX))) / 90) + 1;

				// Calculate position of swerve module based upon current center
				fieldX = DriveTrain.robotPosition()[0] + (Math.cos(Gyro.getUnitCircleAngle() + moduleNum * (Math.atan(DriveTrain.getLength() / DriveTrain.getWidth()))) * (DriveTrain.getRadius() / 2));
				fieldY = DriveTrain.robotPosition()[1] + (Math.sin(Gyro.getUnitCircleAngle() + moduleNum * (Math.atan(DriveTrain.getLength() / DriveTrain.getWidth()))) * (DriveTrain.getRadius() / 2));
			}
